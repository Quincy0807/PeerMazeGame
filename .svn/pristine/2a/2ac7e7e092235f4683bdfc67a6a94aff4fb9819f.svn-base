    ...
    private void joinGame(ActionEvent actionEvent) throws IOException, NotBoundException {
        ...
        if (exchangeData != null) {
            if (player.getRole().equals(Super.backupServer)) {
                LogHelper.logPlayer(player, player.getPlayerIDAsString() + " acts as the backup server");
            } else {
                LogHelper.logPlayer(player, player.getPlayerIDAsString() + " acts as the normal player");
            }
...
//        log("received game information from primary server:\n" + gameInfo + "");
    ...
    public void startMazeGame(ExchangeData exchangeData) {
        lock.lock();
        LogHelper.logPlayer(player, player.getPlayerIDAsString()+" receives start game information from server");
