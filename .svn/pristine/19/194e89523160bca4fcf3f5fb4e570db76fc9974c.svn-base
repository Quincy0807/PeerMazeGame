...
public class Peer  {
    ...
    public Peer(Stage stage, int mazeSize, int treasureNumber) throws IOException, AlreadyBoundException {
        ...
        if (startAsPrimaryServer()) {
            ...
            LogHelper.logPrimary("Primary Server Starts at " + serverAddress + " : " + serverPort);
        }
    ...
    private boolean startAsPrimaryServer() {
        ...
        try {
            ...
            if (Arrays.stream(Inet4Address.getAllByName(hostname)).
                    map(InetAddress::getHostAddress).anyMatch(host -> host.equals(serverAddress))) {
                return false;
            } else {
